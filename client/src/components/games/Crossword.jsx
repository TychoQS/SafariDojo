...
function getCell(cell, row, col, setGrid, grid){
...
const Crossword = ({wordGrid, clues}) =>{
    ...
    const clearHandler = () => {
        const newGrid = [...grid];
        const prevGrid = grid.map(row => row.map(cell => (cell !== null ? "" : null)));
        setGrid(prevGrid);
    };
    ...
    const solveHandler =  () => {
        ...
        if (!isCorrect) console.log("Wrong answer, bitch");
        else console.log("Great answer, bitch");
    }
